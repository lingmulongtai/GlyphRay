...
**Overall progress estimate: 97%**

Last updated: 2026-05-18 JST

```mermaid
pie title Overall Completion
  "Implemented foundation" : 97
  "Remaining product work" : 3
```
...
| Milestone 5 macOS, audio, relay readiness | In progress | 72% |
...
| `hosts/macos-host` | Secondary desktop host | SwiftUI shell, UDP control pairing runtime, LAN discovery advertiser, ScreenCaptureKit display enumeration, live capture probe, live capture-to-VideoToolbox encode probe, H.264 Annex B conversion, GlyphRay video packetizer, manual UDP send probe, continuous UDP video stream start/stop path, permission readiness UI, Keychain smoke test |
...
- macOS live capture has a UDP control pairing listener, LAN discovery advertisement, ScreenCaptureKit frame capture, VideoToolbox encode, GlyphRay Video-channel packetizer, manual UDP send probe, and a continuous UDP video stream start/stop path. It still needs hardened trusted-device identity, reconnect, and backpressure ownership before this becomes the normal session path.
...
- Replace the lightweight macOS pairing/discovery runtime with hardened trusted-device identity, reconnect, and backpressure-aware stream ownership.
