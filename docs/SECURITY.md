...
macOS Keychain now stores approved-client endpoint and public-key metadata, and returning Android devices can prove ownership of the saved public key in the control runtime. Beta still needs migration, backup, corrupted-store recovery tests, encrypted session transport on every media/input path, real-device replay/expiry validation, and UI review of trusted-device permissions.
