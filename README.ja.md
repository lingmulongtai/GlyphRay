...
| Milestone 3 Android stylus から Windows Ink | 進行中 | 56% |
...
| パス | 役割 | 現在入っているもの |
| --- | --- | --- |
| `apps/android-client` | Android client | Compose UI、LAN host discovery、stylus diagnostics、stylus UDP packet 基盤、MediaCodec decode surface |
...
- Android から compact stylus batch を UDP 送信する `GLYT` packet encoder。
...
現在の Android app には、LAN host discovery、stylus diagnostics、session UI、latency overlay、UDP stylus packet sender 基盤、H.264 frame 受信用の MediaCodec-backed decoder surface が入っています。
...
- Android stylus packet は UDP 用に encode できますが、本番 pairing / session handshake はさらに hardening が必要です。
